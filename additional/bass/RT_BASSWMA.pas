{ Original Source : BASSWMA.PAS by Ian Luck
  BASSWMA 2.4 Delphi API, copyright (c) 2002-2006 Ian Luck.
...
const
 ...
 // BASS_CONFIG_WMA_ASX        = $10102;
  ...
  BASS_TAG_WMA               = 8; // WMA header tags : array of null-terminated strings
...
var
 ...
 // BASS_WMA_StreamCreateFileUser : function(flags:DWORD; proc:STREAMFILEPROC; user:DWORD): HSTREAM; stdcall;
  ...
  BASS_WMA_GetWMObject : function(handle:HSTREAM): Pointer; stdcall;
...
{$ENDIF}
   ...
   @BASS_WMA_StreamCreateFileAuth := GetProcAddress(BASSWMA_Handle, 'BASS_WMA_StreamCreateFileAuth');
   @BASS_WMA_StreamCreateFileUser := GetProcAddress(BASSWMA_Handle, 'BASS_WMA_StreamCreateFileUser');
   @BASS_WMA_GetWMObject := GetProcAddress(BASSWMA_Handle, 'BASS_WMA_GetWMObject');
   @BASS_WMA_EncodeGetRates := GetProcAddress(BASSWMA_Handle, 'BASS_WMA_EncodeGetRates');
   @BASS_WMA_EncodeOpen := GetProcAddress(BASSWMA_Handle, 'BASS_WMA_EncodeOpen');
   ...
   @BASS_WMA_EncodeOpenNetworkMulti := GetProcAddress(BASSWMA_Handle, 'BASS_WMA_EncodeOpenNetworkMulti');
   @BASS_WMA_EncodeOpenPublish := GetProcAddress(BASSWMA_Handle, 'BASS_WMA_EncodeOpenPublish');
   @BASS_WMA_EncodeOpenPublishMulti := GetProcAddress(BASSWMA_Handle, 'BASS_WMA_EncodeOpenPublishMulti');
