...
begin
  ...
  begin
    ...
    Info.Tag.FieldData[10] := Trim(FComposer);
    Info.Tag.FieldData[11] := Trim(FLyrics);
    Info.Tag.FieldData[12] := Trim(FURL);
    Info.Tag.FieldData[13] := Trim(FCopyright);
    Info.ExtraTag := FExtraTag;
    Tag := BuildTag(Info);
...
function TOggVorbis.FGetTrackString: string;
begin
  if FTrack = 0 then
    Result := ''
  else
...
procedure TOggVorbis.FSetTrackString(const NewTrack: string);
begin
  FTrack := ExtractTrack(NewTrack);
end;
