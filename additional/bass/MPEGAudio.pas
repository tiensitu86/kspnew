...
type
  ...
  function ReadFromTagStream(TagStream: PAnsiChar; StreamSize: Longword; TagVersion: Word;
...
begin
  ...
  Result.VendorID :=
    AnsiChar(Data[Index + 120]) +
    AnsiChar(Data[Index + 121]) +
    AnsiChar(Data[Index + 122]) +
    AnsiChar(Data[Index + 123]) +
    AnsiChar(Data[Index + 124]) +
    AnsiChar(Data[Index + 125]) +
    AnsiChar(Data[Index + 126]) +
    AnsiChar(Data[Index + 127]);
end;
...
begin
  ...
  //  Result := (MPEGSize - FFrame.Position) div GetFrameLength(FFrame);
    Result := MPEGSize div GetFrameLength(FFrame);  // ** Changed by Silhwan Hyun (2009-05-11)
  end;
end;
...
begin
  ...
  if FFrame.Found then
    ...
    //  Result := (MPEGSize - FFrame.Position) / GetBitRate(FFrame) / 1000 * 8;
      Result := (MPEGSize / GetBitRate(FFrame)) / 1000 * 8;  // ** Changed by Silhwan Hyun (2009-05-11)
    end
  else
    Result := 0;
end;
...
begin
  ...
  if (FVBR.ID = VBR_ID_XING) and
    (Vendor_ID <> VENDOR_ID_LAME) and
    (Vendor_ID <> VENDOR_ID_GOGO_NEW) and
    (Vendor_ID <> VENDOR_ID_GOGO_OLD) then
...
begin
    ...
    try
      ...
      Result := true;
        ...
        if (SourceFile <> hINVALID_HANDLE_VALUE) then CloseHandle(SourceFile);
        Result := false;
    end;
