...
type
  ...
  TGeckoBrowserHisoty = record
    ...
    //TODO 2 -cTCustomGeckoBrowser: DocShell vçpeBÇí«â¡
...
function TGeckoBrowserChrome.GetChromeFlags
                : PRUint32;
begin
  //TODO 2 -cTGeckoBrowserChrome: Chrome tâOÇÃµÇ¢ÇÇ«Ç§ÇµÇÊÇ§Ç©
...
begin
  //TODO 2 -cTGeckoBrowserChrome: TGeckoBrowserChrome.OnDestroyBrowser CxìgÇÃí«â¡
end;
...
begin
  // TODO 1 -cTGeckoBrowserChrome: TGeckoBrowserChrome.GetVisibility ÇÕÇ«Ç§Ç∑Ç◊Ç´Ç©
  Result := True;
end;
...
procedure TGeckoBrowserChrome.SetVisibility(
                aVisibility: PRBool);
begin
  //TODO 1 -cTGeckoBrowserChrome: TGeckoBrowserChrome.SetVisibility ÇÃé¿ëï
end;
...
begin
  ...
  begin
    // èÛë‘ÇÃïœâªÇÕhLÖìgÇ…ëŒÇµÇƒÇ≈Ç†ÇÈ
    if (aStateFlags and STATE_START)<>0 then
    begin
      // hLÖìgÇÃì«Ç›çûÇ›Ç™äJénÇ≥ÇÍÇΩ
      {$IFDEF DEBUG}
      {
  ...
  begin
    // èÛë‘ÇÃïœâªÇÕlbgè[NÇ…ëŒÇµÇƒÇ≈Ç†ÇÈ
    if (aStateFlags and STATE_START)<>0 then
    begin
      // lbgè[NÇÃì]ëóÇ™äJénÇ≥ÇÍÇΩèÍçá
      {$IFDEF DEBUG}
      {
      str := NewCString;
...
procedure TGeckoBrowserListener.OnSecurityChange(
                ...
                state: PRUint32);
begin
  //TODO 1 -cTGeckoBrowserListner: TGeckoBrowserListener.OnSecurityChange ÇÃãLèq
end;
...
begin
  ...
  begin
    // nsIDOMWindow ÇèóùÇµÇ»Ç¢Ç∆ nsIWindowCreator.CreateChromeWindow Ç≈Gâ[Ç…Ç»ÇÈ
